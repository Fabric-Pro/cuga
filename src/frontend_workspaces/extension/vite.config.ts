...
async function copyAndDeleteFile(oldFilePath) {
	...
	try {
		...
		if (err.code === "EACCES") {
			...
			console.error(
				`Error copying and deleting file ${oldFilePath}:`,
				err,
			);
		}
	}
...
function generateManifest(mode: string, env: { [x: string]: string }) {
	...
	const description = isProductionMode
		...
		: `${target}: server "${env["NL2UI_SERVER_HOST"]}" Auth: ${env["NL2UI_AUTH_TYPE"]}`.slice(
				0,
				MAX_DESCRIPTION,
			);
		...
		return {
			...
			plugins: [
				webExtension({
					manifest: () => manifest,
					additionalInputs: [
						"src/assets/sidepanel.html",
						"src/logWorker/log.worker.ts",
					],
					browser: target,
				}),
				viteStaticCopy({
					targets: [
						...
						{
							src: normalizePath(
								"./src/assets/*.(png|jpeg|yaml)",
							),
							dest: normalizePath("assets"),
						},
						{
							src: normalizePath(
								"../node_modules/@ibm/plex/IBM-Plex-Sans",
							),
			...
			resolve: {
				alias: {
					"@uiagent/shared": path.resolve(
						__dirname,
						"../shared/dist/index.js",
					),
					"@agentic_chat": path.resolve(
						__dirname,
						"../agentic_chat/src",
					),
				},
