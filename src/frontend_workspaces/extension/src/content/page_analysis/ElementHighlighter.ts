...
export class ElementHighlighter {
	...
	public highlightElement(
		...
		try {
			...
			labelHeight =
				label.offsetHeight > 0 ? label.offsetHeight : labelHeight; // Update actual height if possible
			...
			const updatePositions = (): void => {
				...
				if (label && newRects.length > 0) {
					...
					const firstNewRectTop =
						firstNewRect.top + newIframeOffset.y;
					const firstNewRectLeft =
						firstNewRect.left + newIframeOffset.x;
					...
					if (
						...
						newLabelLeft =
							firstNewRectLeft + firstNewRect.width - labelWidth;
			...
			const throttledUpdatePositions = throttleFunction(
				updatePositions,
				16,
			); // ~60fps
			...
			if (cleanupFn) {
				...
				(windowWithCleanup._highlightCleanupFunctions =
					windowWithCleanup._highlightCleanupFunctions || []).push(
					cleanupFn,
				);
			}
		}
