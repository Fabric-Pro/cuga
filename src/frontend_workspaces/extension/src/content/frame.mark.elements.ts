...
export class FrameMarkElementsModule {
	...
	private generateElementId(frameId: string): string {
		const fullId = frameId
			? frameId + ":" + this.elementIdCounter
		...
		try {
			...
			for (const element of allElements) {
				try {
					...
					element.setAttribute(
						BROWSERGYM_VISIBILITY_ATTRIBUTE,
						visibility.toString(),
					);
					...
					warnings.push(
						`Failed to mark element ${element.tagName}: ${(error as Error).message}`,
					);
				}
			}
			...
			for (const iframe of iframes) {
				try {
					...
					warnings.push(
						`Failed to mark iframe: ${(error as Error).message}`,
					);
				}
			}
		} catch (error) {
		...
		try {
			...
			if (tagName === "option") {
				...
				element.setAttribute(
					...
					optionElement.selected.toString(),
				);
			}
			...
			console.warn(
				`Failed to store dynamic properties for ${tagName}:`,
				error,
			);
		}
	}
	...
	public unmarkElements(): void {
		try {
			const elementsWithBid = document.querySelectorAll(
	...
	public getFocusedElementBid(): string {
		try {
			...
			if (focusedElement) {
				return (
					focusedElement.getAttribute(BROWSERGYM_ID_ATTRIBUTE) || ""
				);
			}
					...
					switch (request.type) {
						...
						case "get_element_rect": {
							try {
								const element = document.querySelector(
									`[${BROWSERGYM_ID_ATTRIBUTE}="${request.bid}"]`,
								);
								if (!element) {
									throw new Error(
										`Element with dom-tree-id '${request.bid}' not found`,
									);
								}
								const rect = (
						...
						case "extract_dom_tree":
							try {
								...
								const domTreeAPI =
									(window as any).DOMTreeAPI ||
									(window as any).CUGA_DOMTreeAPI;
								if (!domTreeAPI) {
									throw new Error(
								...
								const result = domTreeAPI.analyzePage({
									doHighlightElements:
										request.data?.doHighlightElements ||
										false,
									focusHighlightIndex:
										request.data?.focusHighlightIndex || -1,
									viewportExpansion:
										request.data?.viewportExpansion || 0,
						...
						case "select_option": {
							...
							(async () => {
								try {
									...
									await this.performSelectOption(
										bid,
										options,
									);
						...
						case "add_animation": {
							...
							const result = addAnimationToElement(
								bid,
								iconType,
								bannerText,
							);
	 ...
	 */
	private async performSelectOption(
		bid: string,
