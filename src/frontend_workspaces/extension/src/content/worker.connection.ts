...
export class WorkerConnection {
	...
	private async onMessage(command: Command): Promise<void> {
		try {
			...
			if (command.type == undefined)
				throw Error(
					`The command 'type' is required and it was not specified: ${JSON.stringify(command)}`,
				);
			if (command.id == undefined || command.id == "")
				throw Error(
					`The command 'id' is required and it was not specified: ${JSON.stringify(command)}`,
				);

			if (command.type == "pu.connection.keep-alive") {
				this.port?.postMessage(
					new responses.Response(command.id, true),
				);
				return;
			}
			...
			this.port!.postMessage(
				new responses.Response(command.id, undefined, error),
			);
		}
	}
	...
	public connect(): void {
		...
		["pageshow", "focus", "blur", "visibilitychange", "resume"].forEach(
			(type) => {
				window.addEventListener(type, handleStateChange, {
					capture: true,
				});
			},
		);
	...
	private async getPortClosed() {
		return new Promise<boolean>((resolve) => {
			...
			try {
				...
				this.port.postMessage({
					id: id,
					type: "nl2ui.page.internal.checkportstatus",
				});
				resolve(true);
			} catch (error: any) {
