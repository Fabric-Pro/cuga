...
export class HttpStreamModule {
	...
	private connectToCommandStream(): void {
		...
		this.eventSource.onmessage = async (event) => {
			try {
				...
				if (
					cmd.type === "agent_response" ||
					cmd.type === "agent_complete" ||
					cmd.type === "agent_error"
				) {
					// If this was initiated by a popup, resolve the pending promise
					if (
						cmd.request_id &&
						this.pendingAgentQueries.has(cmd.request_id)
					) {
						const resolver = this.pendingAgentQueries.get(
							cmd.request_id,
						);
						if (resolver) {
							resolver(cmd);
		...
		try {
			switch (command) {
				...
				default:
					return {
						type: "error",
						message: `Unknown browser command: ${command}`,
					};
			}
		} catch (e: any) {
	...
	private async handleClickCommand(args: any): Promise<any> {
		...
		console.log(
			`[CUGA] Clicking element with BID: ${bid}, button: ${button}, modifiers: ${modifiers}`,
		);
				...
				if (forIndex !== -1 && forIndex + 1 < attrs.length) {
					...
					if (forValue) {
						...
						if (queryRes && queryRes.nodeId) {
							console.log(
								`[CUGA] Redirecting click from <label> to associated element id '${forValue}', nodeId: ${queryRes.nodeId}`,
							);
		...
		if (elementRect) {
			...
			console.log(
				`[CUGA] Bounding rect via content script, center: (${centerX}, ${centerY})`,
			);
		}
		...
		if (centerX === null || centerY === null) {
			...
			console.log(
				`[CUGA] Fallback BoxModel center: (${centerX}, ${centerY})`,
			);
		}
	...
	private async handleTypeCommand(args: any): Promise<any> {
		...
		console.log(
			`[CUGA] Typing in element with BID: ${bid}, value: "${value}", press_enter: ${press_enter}`,
		);
	...
	private async handleAddAnimation(args: any): Promise<any> {
		...
		const animResult = await (globalThis as any).chrome.tabs.sendMessage(
			activeTab.id,
			{
				type: "add_animation",
				bid,
	...
	private async handleSelectOption(args: any): Promise<any> {
		...
		const selectResult = await (globalThis as any).chrome.tabs.sendMessage(
			activeTab.id,
			{
				type: "select_option",
				bid,
	...
	private async handleMarkElements(data: any): Promise<any> {
		try {
			...
			const response = await (globalThis as any).chrome.tabs.sendMessage(
				activeTab.id,
				{
					type: "mark_elements",
					data: {
		...
		try {
			...
			try {
				const cleanupDelayMs =
					data && typeof data.cleanup_delay_ms === "number"
						? data.cleanup_delay_ms
						: 3000;
		...
		try {
			...
			const response = await (globalThis as any).chrome.tabs.sendMessage(
				activeTab.id,
				{
					...
					data: {
						doHighlightElements:
							data.do_highlight_elements || false,
	...
	// --- Utility methods ---
	private async findElementByDomTreeId(
		tabId: number,
		bid: string,
	): Promise<number | null> {
		...
		console.log(
			`[CUGA] Found element with BID '${bid}', nodeId: ${nodeId}`,
		);
		return nodeId;
	}
		...
		try {
			...
			try {
				await new Promise((resolve, reject) => {
					const timeout = setTimeout(() => {
						reject(
							new Error("Content script verification timeout"),
						);
					}, 2000);
				...
				throw new Error(
					`Content script not available: ${(verificationError as Error).message}`,
				);
			}
		}
	}
	...
	private async ensureDebuggerAttached(tabId: number): Promise<void> {
		try {
			await (globalThis as any).chrome.debugger.attach(
				{ tabId: tabId },
				"1.3",
			);
		} catch (error) {
			...
			if (
				!(
					error &&
					((error as Error).message?.includes("already attached") ||
						(error as Error).message?.includes("Already attached"))
				)
			) {
	...
	private async handleAgentQuery(query: string): Promise<any> {
		...
		try {
			const response = await fetch(
				`${SERVER_BASE}/extension/agent_query`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
			...
			if (!response.ok) {
				throw new Error(
					`Failed to send agent query: ${response.statusText}`,
				);
			}
