...
function gr() {
	...
	class _ {
		constructor(a, m) {
			(this.buffer = ""),
				(this.classPrefix = m.classPrefix),
				a.walk(this);
		}
		addText(a) {
			this.buffer += t(a);
		...
		openNode(a) {
			if (!c(a)) return;
			const m = b(a.scope, { prefix: this.classPrefix });
			this.span(m);
		}
		closeNode(a) {
			c(a) && (this.buffer += d);
	...
	class o {
		...
		static _collapse(a) {
			typeof a != "string" &&
				a.children &&
				(a.children.every((m) => typeof m == "string")
	...
	const D = /\b\B/,
		...
		ee = (n = {}) => {
			const a = /^#![ ]*\//;
			return (
				n.binary && (n.begin = y(a, /.*\b/, n.binary, /\b.*/)),
				s(
					{
						scope: "meta",
						begin: a,
	...
	const Ke = (n, a) => {
			...
			Object.keys(n).forEach((C) => {
				...
				(n.starts = {
					relevance: 0,
					contains: [Object.assign(m, { endsParent: !0 })],
				}),
				(n.relevance = 0),
	...
	const He = {},
		...
		ue = (n, a) => {
			He[`${n}/${a}`] ||
				(console.log(`Deprecated as of ${n}. ${a}`),
				(He[`${n}/${a}`] = !0));
		},
		Le = new Error();
	function We(n, a, { key: m }) {
		let C = 0;
		const V = n[m],
			Q = {},
			S = {};
		for (let v = 1; v <= a.length; v++)
			(S[v + C] = V[v]), (Q[v + C] = !0), (C += O(a[v - 1]));
	...
	function Ye(n) {
		if (Array.isArray(n.begin)) {
			...
			if (typeof n.beginScope != "object" || n.beginScope === null)
				throw (ve("beginScope must be object"), Le);
			We(n, n.begin, { key: "beginScope" }),
				(n.begin = x(n.begin, { joinWith: "" }));
		}
	}
	function Pe(n) {
		if (Array.isArray(n.end)) {
			...
			if (typeof n.endScope != "object" || n.endScope === null)
				throw (ve("endScope must be object"), Le);
			We(n, n.end, { key: "endScope" }),
				(n.end = x(n.end, { joinWith: "" }));
		}
	}
	function on(n) {
		n.scope &&
	...
	function Ze(n) {
		on(n),
			typeof n.beginScope == "string" &&
				(n.beginScope = { _wrap: n.beginScope }),
			typeof n.endScope == "string" &&
				(n.endScope = { _wrap: n.endScope }),
			Ye(n),
			Pe(n);
	}
	function sn(n) {
		...
		class m {
			...
			compile() {
				...
				(this.matcherRe = a(x(v, { joinWith: "|" }), !0)),
					(this.lastIndex = 0);
			}
			exec(v) {
				...
				if (!k) return null;
				const ae = k.findIndex((ze, un) => un > 0 && ze !== void 0),
					j = this.matchIndexes[ae];
				return k.splice(0, ae), Object.assign(k, j);
			}
		}
		class C {
			constructor() {
			...
			getMatcher(v) {
				...
				const k = new m();
				return (
					this.rules.slice(v).forEach(([ae, j]) => k.addRule(ae, j)),
					k.compile(),
					(this.multiRegexes[v] = k),
					k
				);
			}
			...
			exec(v) {
				...
				if (
					...
					!(ae && ae.index === this.lastIndex)
				) {
					const j = this.getMatcher(0);
					(j.lastIndex = this.lastIndex + 1), (ae = j.exec(v));
				}
				return (
					ae &&
						((this.regexIndex += ae.position + 1),
		...
		function V(S) {
			const v = new C();
			return (
				S.contains.forEach((k) =>
					v.addRule(k.begin, { rule: k, type: "begin" }),
				),
		...
		return (n.classNameAliases = s(n.classNameAliases || {})), Q(n);
	}
	function Xe(n) {
	...
	class dn extends Error {
		constructor(a, m) {
			super(a), (this.name = "HTMLInjectionError"), (this.html = m);
		}
	}
	const Fe = t,
		Ve = s,
...
https://github.com/highlightjs/highlight.js/issues/2277`,
			...
			function ze(u, A, P, q) {
					...
					for (; w; ) {
						...
						if (ce) {
							...
							if (
								...
								const dr = he.classNameAliases[Te] || Te;
								Ee(w[0], dr);
							}
						} else L += w[0];
						(h = I.keywordPatternRe.lastIndex),
							(w = I.keywordPatternRe.exec(H));
					}
					(L += H.substring(h)), se.addText(L);
				}
				function je() {
					if (H === "") return;
					let h = null;
					if (typeof I.subLanguage == "string") {
						...
						(h = ze(I.subLanguage, H, !0, ct[I.subLanguage])),
							(ct[I.subLanguage] = h._top);
					} else
						h = gn(H, I.subLanguage.length ? I.subLanguage : null);
					I.relevance > 0 && (nn += h.relevance),
						se.__addSublanguage(h._emitter, h.language);
				}
				function be() {
				...
				function Ee(h, w) {
					h !== "" &&
						(se.startScope(w), se.addText(h), se.endScope());
				}
				function it(h, w) {
					let L = 1;
					const $ = w.length - 1;
				...
				function at(h, w) {
					return (
						h.scope &&
							...
							se.openNode(
								he.classNameAliases[h.scope] || h.scope,
							),
						h.beginScope &&
							(h.beginScope._wrap
								? (Ee(
				...
				function ot(h, w, L) {
					...
					if (h.endsWithParent) return ot(h.parent, w, L);
				}
				function ir(h) {
				...
				function ar(h) {
					...
					for (const Te of ce)
						if (Te && (Te(h, $), $.isMatchIgnored)) return ir(w);
					return (
						L.skip
							? (H += w)
							: (L.excludeBegin && (H += w),
								be(),
								!L.returnBegin && !L.excludeBegin && (H = w)),
						at(L, h),
				...
				function or(h) {
					...
					I.endScope && I.endScope._wrap
						...
						: I.endScope && I.endScope._multi
							...
							: ce.skip
								? (H += w)
								: (ce.returnEnd || ce.excludeEnd || (H += w),
									be(),
									ce.excludeEnd && (H = w));
					do
						I.scope && se.closeNode(),
							!I.skip && !I.subLanguage && (nn += I.relevance),
							(I = I.parent);
					while (I !== $.parent);
					return (
						$.starts && at($.starts, h), ce.returnEnd ? 0 : w.length
					);
				}
				function sr() {
					const h = [];
					for (let w = I; w !== he; w = w.parent)
						w.scope && h.unshift(w.scope);
					h.forEach((w) => se.openNode(w));
				}
				let en = {};
				function st(h, w) {
					const L = w && w[0];
					...
					if (
						...
						if (((H += A.slice(w.index, w.index + 1)), !V)) {
							const $ = new Error(`0 width match regex (${u})`);
							throw (
								(($.languageName = u), ($.badRule = en.rule), $)
							);
						}
						return 1;
					}
					if (((en = w), w.type === "begin")) return ar(w);
					if (w.type === "illegal" && !P) {
						const $ = new Error(
							'Illegal lexeme "' +
								L +
...
`),
				...
				let H = "",
					...
					_n = !1;
				try {
					if (he.__emitTokens) he.__emitTokens(A, se);
					else {
						for (I.matcher.considerAll(); ; ) {
							mn++,
								_n ? (_n = !1) : I.matcher.considerAll(),
								(I.matcher.lastIndex = xe);
			...
			function gn(u, A) {
				...
				const te = q.sort((be, Ee) => {
						if (be.relevance !== Ee.relevance)
							return Ee.relevance - be.relevance;
						if (be.language && Ee.language) {
							if (Ae(be.language).supersetOf === Ee.language)
								return 1;
							if (Ae(Ee.language).supersetOf === be.language)
								return -1;
						}
						return 0;
					}),
			...
			function Yt(u, A, P) {
				const q = (A && m[A]) || P;
				u.classList.add("hljs"), u.classList.add(`language-${q}`);
			}
			function bn(u) {
				let A = null;
				const P = ae(u);
				if (k(P)) return;
				if (
					(Je("before:highlightElement", { el: u, language: P }),
				...
				const q = A.textContent,
					te = P ? j(q, { language: P, ignoreIllegals: !0 }) : gn(q);
			...
			function Qe() {
				...
				if (document.readyState === "loading") {
					nt || window.addEventListener("DOMContentLoaded", u, !1),
						(nt = !0);
					return;
				}
				document.querySelectorAll(v.cssSelector).forEach(bn);
			}
			function Qt(u, A) {
				let P = null;
				try {
				...
				P.name || (P.name = u),
					...
					P.aliases && tt(P.aliases, { languageName: u });
			}
			function Jt(u) {
				delete a[u];
			...
			function Ae(u) {
				return (u = (u || "").toLowerCase()), a[u] || a[m[u]];
			}
			function tt(u, { languageName: A }) {
				typeof u == "string" && (u = [u]),
					u.forEach((P) => {
						m[P.toLowerCase()] = A;
					});
			...
			for (const u in ie) typeof ie[u] == "object" && r(ie[u]);
			return Object.assign(n, ie), n;
		},
		Oe = we({});
	return (
		(Oe.newInstance = () => we({})),
		...
		return {
			...
			contains: [
				...
				{ className: "tag", begin: /<>|<\/>/ },
				{
					...
					begin: t.concat(
						/</,
						t.lookahead(t.concat(s, t.either(/\/>/, />/, /\s/))),
					),
					end: /\/?>/,
					contains: [
						{
	...
	function r(e) {
		const t = e.regex,
			...
			d = {
				...
				contains: ["self", { begin: /:-/, contains: [s] }],
			};
		Object.assign(s, {
			className: "variable",
			variants: [
		...
		const c = {
			...
			p = {
				...
				contains: [e.BACKSLASH_ESCAPE, s, c],
			};
		c.contains.push(p);
		const o = { match: /\\"/ },
			...
			E = e.SHEBANG({ binary: `(${f.join("|")})`, relevance: 10 }),
			y = {
				className: "function",
				begin: /\w[\w\d_]*\s*\(\s*\)\s*\{/,
				returnBegin: !0,
		...
		return {
			...
			contains: [E, e.SHEBANG(), y, g, b, _, O, p, o, i, l, s],
		};
	}
	return (hn = r), hn;
}
...
function mr() {
	...
	function r(e) {
		const t = e.regex,
			...
			O = [g, p, s, e.C_BLOCK_COMMENT_MODE, l, i],
			R = {
				variants: [
					{ begin: /=/, end: /;/ },
					{ begin: /\(/, end: /\)/ },
			...
			F = {
				...
				contains: [
					{ begin: d, keywords: T, relevance: 0 },
					{
						...
						returnBegin: !0,
						contains: [
							e.inherit(f, { className: "title.function" }),
						],
						relevance: 0,
					},
					{ relevance: 0, match: /,/ },
		...
		return {
			...
			exports: { preprocessor: g, strings: i, keywords: T },
		};
	}
	return (Nn = r), Nn;
}
...
function _r() {
	...
	function r(e) {
		const t = e.regex,
			...
			B = [D, g, p, s, e.C_BLOCK_COMMENT_MODE, l, i],
			M = {
				variants: [
					{ begin: /=/, end: /;/ },
					{ begin: /\(/, end: /\)/ },
			...
			U = {
				...
				contains: [
					...
					{ begin: /::/, relevance: 0 },
					{ begin: /:/, endsWithParent: !0, contains: [i, l] },
					{ relevance: 0, match: /,/ },
					{
						className: "params",
						begin: /\(/,
...
function fr() {
	...
	function r(e) {
		...
		const O = {
				variants: [i, N, y, l, e.APOS_STRING_MODE, e.QUOTE_STRING_MODE],
			},
			R = {
				begin: "<",
				end: ">",
				contains: [{ beginKeywords: "in out" }, p],
			},
			F =
				e.IDENT_RE +
				"(<" +
		...
		return {
			...
			contains: [
				...
				{ beginKeywords: "new return throw await else", relevance: 0 },
				{
					...
					keywords: _,
					contains: [
						{ beginKeywords: s.join(" "), relevance: 0 },
						{
						...
						{ match: /\(\)/ },
						{
							...
							keywords: _,
							relevance: 0,
							contains: [O, o, e.C_BLOCK_COMMENT_MODE],
						},
						e.C_LINE_COMMENT_MODE,
						e.C_BLOCK_COMMENT_MODE,
					],
...
function Er() {
	...
	const r = (o) => ({
			...
			ATTRIBUTE_SELECTOR_MODE: {
				...
				illegal: "$",
				contains: [o.APOS_STRING_MODE, o.QUOTE_STRING_MODE],
			},
			CSS_NUMBER_MODE: {
				scope: "number",
				begin:
	...
	function p(o) {
		const i = o.regex,
			...
			E = /@-?\w[\w]*(-\w+)*/,
			y = "[a-zA-Z-][a-zA-Z0-9_-]*",
			N = [o.APOS_STRING_MODE, o.QUOTE_STRING_MODE];
		return {
			name: "CSS",
			case_insensitive: !0,
			illegal: /[=|'\$]/,
...
function hr() {
	...
	function r(e) {
		const t = e.regex,
			...
			g = e.inherit(i, { contains: [] }),
			f = e.inherit(l, { contains: [] });
		i.contains.push(f), l.contains.push(g);
		let E = [s, o];
		return (
			[i, l, g, f].forEach((O) => {
		...
		return {
			...
			contains: [
				...
				{ className: "section", begin: /\[+/, end: /\]+/ },
				{
					begin: f,
					className: "attr",
					starts: { end: /$/, contains: [d, p, b, c, _, s] },
				},
			],
		};
	}
...
function wr() {
	...
	var r = "[0-9](_*[0-9])*",
		...
		s = {
			...
			variants: [
				{
					begin: `(\\b(${r})((${e})|\\.)?|(${e}))[eE][+-]?(${r})[fFdD]?\\b`,
				},
				{ begin: `\\b(${r})((${e})[fFdD]?\\b|\\.([fFdD]\\b)?)` },
				{ begin: `(${e})[fFdD]?\\b` },
				{ begin: `\\b(${r})[fFdD]\\b` },
				{
					begin: `\\b0[xX]((${t})\\.?|(${t})?\\.(${t}))[pP][+-]?(${r})[fFdD]?\\b`,
				},
	...
	function d(b, _, p) {
		return p === -1 ? "" : b.replace(_, (o) => d(b, _, p - 1));
	}
	function c(b) {
		const _ = b.regex,
...
function Or() {
	...
	function p(o) {
		...
		function Ne(X) {
			return i.concat("(?!", X.join("|"), ")");
		}
		const Se = {
				match: i.concat(
					/\b/,
					Ne([...c, "super", "import"].map((X) => `${X}\\s*\\(`)),
					g,
					i.lookahead(/\s*\(/),
				),
			...
			Ce = {
				begin: i.concat(
					/\./,
					i.lookahead(i.concat(g, /(?![0-9A-Za-z$_(])/)),
				),
				end: g,
				excludeBegin: !0,
				keywords: "prototype",
		...
		return {
			...
			contains: [
				...
				F,
				J,
				{ scope: "attr", match: g + i.lookahead(":"), relevance: 0 },
				ye,
				{
					begin:
						"(" +
	...
	function d(c) {
		const b = {
			...
			o = {
				...
				contains: [c.C_NUMBER_MODE],
			},
			i = { className: "variable", begin: "\\$" + c.UNDERSCORE_IDENT_RE },
			l = {
				className: "string",
				variants: [
					...
					},
					{
						...
						illegal: /\n/,
						contains: [c.BACKSLASH_ESCAPE, i, o],
					},
				],
			};
		o.contains.push(l);
		const g = {
			...
			E = s,
			y = c.COMMENT("/\\*", "\\*/", {
				contains: [c.C_BLOCK_COMMENT_MODE],
			}),
			N = {
				variants: [
...
function kr() {
	...
	function r(e) {
		const t = "\\[=*\\[",
			s = "\\]=*\\]",
			d = { begin: t, end: s, contains: ["self"] },
			c = [
				e.COMMENT("--(?!" + t + ")", "$"),
				e.COMMENT("--" + t, s, { contains: [d], relevance: 10 }),
			];
		return {
			name: "Lua",
			aliases: ["pluto"],
...
function Cr() {
	...
	function r(e) {
		...
		return {
			...
			contains: [e.HASH_COMMENT_MODE, t, s, d, c, b, _],
		};
	}
	return (Bn = r), Bn;
}
...
function Ir() {
	...
	function r(e) {
		...
		return (
			(b.contains = y),
			(_.contains = y),
			{ name: "Perl", aliases: ["pl", "pm"], keywords: c, contains: y }
		);
	}
	return (Un = r), Un;
}
...
function Lr() {
	...
	function r(e) {
		const t = e.regex,
			s = /(?![A-Za-z0-9])(?![$])/,
			d = t.concat(/[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*/, s),
			c = t.concat(
			...
			y = { scope: "string", variants: [l, i, g, f] },
			N = {
				scope: "number",
				variants: [
			...
			G = {
				...
				keywords: x,
				contains: [K, _, U, e.C_BLOCK_COMMENT_MODE, y, N, B],
			},
			re = {
				relevance: 0,
				match: [
			...
			g = {
				...
				variants: [
					...
					},
					{
						...
						contains: [e.BACKSLASH_ESCAPE, o, l, i],
					},
					{
						begin: /([fF][rR]|[rR][fF]|[fF])"""/,
						end: /"""/,
					...
					{ begin: /([bB]|[bB][rR]|[rR][bB])"/, end: /"/ },
					{
						...
						contains: [e.BACKSLASH_ESCAPE, l, i],
					},
					{
						begin: /([fF][rR]|[rR][fF]|[fF])"/,
						end: /"/,
			...
			O = {
				...
				variants: [
					{ className: "", begin: /\(\s*\)/, skip: !0 },
					{
						...
						excludeEnd: !0,
						keywords: p,
						contains: ["self", o, N, g, e.HASH_COMMENT_MODE],
					},
				],
			};
		return (
	...
	function r(e) {
		...
		return {
			...
			contains: [
				...
				{ scope: { 3: "operator" }, match: [s, /\s+/, /<-/, /\s+/] },
				{
					scope: "operator",
					relevance: 0,
					variants: [{ match: c }, { match: /%[^%]*%/ }],
				},
...
function zr() {
	...
	function r(e) {
		const t = e.regex,
			s = /(r#)?/,
			d = t.concat(s, e.UNDERSCORE_IDENT_RE),
	...
	function p(o) {
		...
		return {
			...
			contains: [
				...
				N,
				{ begin: /\(/, end: /\)/, contains: [i.CSS_NUMBER_MODE] },
				i.CSS_VARIABLE,
				{
					className: "attribute",
					begin: "\\b(" + _.join("|") + ")\\b",
				...
				},
				{
					...
					keywords: { $pattern: f, keyword: "@page @font-face" },
				},
				{
					begin: "@",
					end: "[{;]",
...
function Gr() {
	...
	function r(e) {
		...
		function R(D) {
			return t.concat(
				/\b/,
				t.either(...D.map((B) => B.replace(/\s+/, "\\s+"))),
				/\b/,
			);
		}
...
function qr() {
	...
	function s(M) {
		...
		return typeof U == "object" && U.constructor === Object
			? (M.splice(M.length - 1, 1), U)
			: {};
	}
	function d(...M) {
		return (
	...
	function B(M) {
		const U = { match: /\s+/, relevance: 0 },
			...
			ee = { match: t(/\./, d(...o)), relevance: 0 },
			ne = o.filter((z) => typeof z == "string").concat(["_|0"]),
			W = o
			...
			Be = (z) => {
				...
				return {
					...
					contains: [
						...$e,
						{ scope: "comment", begin: `#(?!.*${we})`, end: /$/ },
					],
				};
			},
			Ke = {
				scope: "regexp",
				variants: [Be("###"), Be("##"), Be("#"), tn],
			},
			Ge = { match: t(/`/, R, /`/) },
			rn = { className: "variable", match: /\$\d+/ },
			qe = { className: "variable", match: `\\$${O}+` },
			Ue = [Ge, rn, qe],
			an = {
				match: /(@|#(un)?)available/,
				scope: "keyword",
		...
		const We = { match: t(R, /\s*:/), keywords: "_|0", relevance: 0 },
			...
			Ze = {
				...
				keywords: J,
				contains: [on, ...G, ...oe, ...pe, X, _e, ...Ie, ue, Ye],
				endsParent: !0,
				illegal: /["']/,
			},
			sn = {
			...
			Ve = {
				...
				contains: [
					...
					...oe,
					{
						...
						keywords: J,
						contains: [
							{ scope: "title.class.inherited", match: F },
							...oe,
						],
						relevance: 0,
					},
		...
		for (const z of _e.variants) {
			...
			De.keywords = J;
			const we = [...oe, ...Ne, ...pe, X, _e, ...Ue];
			De.contains = [
				...we,
				{ begin: /\(/, end: /\)/, contains: ["self", ...we] },
			];
		}
		return {
			name: "Swift",
...
function Wr() {
	...
	function p(i) {
		const l = i.regex,
			g = (Y, { after: le }) => {
				const ie = "</" + Y[0].slice(1);
				return Y.input.indexOf(ie, le) !== -1;
			},
			f = r,
			E = { begin: "<>", end: "</>" },
			y = /<[A-Za-z0-9\\._:-]+\s*\/>/,
			N = {
				...
				isTrulyOpeningTag: (Y, le) => {
					const ie = Y[0].length + Y.index,
						de = Y.input[ie];
					if (de === "<" || de === ",") {
						le.ignoreMatch();
						return;
					}
					de === ">" && (g(Y, { after: ie }) || le.ignoreMatch());
					let ge;
					const _e = Y.input.substring(ie);
					if ((ge = _e.match(/^\s*=/))) {
						le.ignoreMatch();
						return;
					}
		...
		const Ce = {
			...
			pe = {
				begin: l.concat(
					/\./,
					l.lookahead(l.concat(f, /(?![0-9A-Za-z$_(])/)),
				),
				end: f,
				excludeBegin: !0,
				keywords: "prototype",
		...
		return {
			...
			aliases: ["js", "jsx", "mjs", "cjs"],
			keywords: T,
			exports: { PARAMS_CONTAINS: W, CLASS_REFERENCE: oe },
			illegal: /#(?![$_A-z])/,
			contains: [
...
function Yr() {
	...
	function r(e) {
		const t = e.regex,
			s = { className: "string", begin: /"(""|[^/n])"C\b/ },
			d = {
				className: "string",
				begin: /"/,
				end: /"/,
...
function Xr() {
	...
	var r = gr();
	return (
		r.registerLanguage("xml", br()),
		r.registerLanguage("bash", pr()),
		r.registerLanguage("c", mr()),
		r.registerLanguage("cpp", _r()),
		r.registerLanguage("csharp", fr()),
		r.registerLanguage("css", Er()),
		r.registerLanguage("markdown", hr()),
		r.registerLanguage("diff", Nr()),
		r.registerLanguage("ruby", yr()),
		r.registerLanguage("go", vr()),
		r.registerLanguage("graphql", Tr()),
		r.registerLanguage("ini", Sr()),
		r.registerLanguage("java", wr()),
		r.registerLanguage("javascript", Or()),
		r.registerLanguage("json", Ar()),
		r.registerLanguage("kotlin", Rr()),
		r.registerLanguage("less", Mr()),
		r.registerLanguage("lua", kr()),
		r.registerLanguage("makefile", Cr()),
		r.registerLanguage("perl", Ir()),
		r.registerLanguage("objectivec", xr()),
		r.registerLanguage("php", Lr()),
		r.registerLanguage("php-template", Dr()),
		r.registerLanguage("plaintext", Br()),
		r.registerLanguage("python", Ur()),
		r.registerLanguage("python-repl", Pr()),
		r.registerLanguage("r", Fr()),
		r.registerLanguage("rust", zr()),
		r.registerLanguage("scss", $r()),
		r.registerLanguage("shell", Kr()),
		r.registerLanguage("sql", Gr()),
		r.registerLanguage("swift", qr()),
		r.registerLanguage("yaml", Hr()),
		r.registerLanguage("typescript", Wr()),
		r.registerLanguage("vbnet", Yr()),
		r.registerLanguage("wasm", Zr()),
		(r.HighlightJS = r),
		(r.default = r),
		(et = r),
		et
...
var Vr = Xr();
const Jr = ur(Vr);
export { Jr as HighlightJS, Jr as default };
