...
const CardManager: React.FC<CardManagerProps> = ({ chatInstance }) => {
	...
	useEffect(() => {
		...
		currentSteps.forEach((step) => {
			...
			console.log(
				"[Variables Debug] Processing Answer/FinalAnswerAgent step",
			);
			...
			try {
				...
				if (typeof step.content === "string") {
					try {
						...
						console.log(
							"[Variables Debug] Failed to parse JSON:",
							e,
						);
					}
				} else if (
				...
				if (Object.keys(variables).length > 0) {
					...
					console.log(
						"[Variables Debug] No variables found in this step",
					);
				}
			} catch (e) {
			...
			const hasChanges = prev.some((entry, index) => {
				...
				return (
					...
					JSON.stringify(entry.variables) !==
						JSON.stringify(newEntry.variables)
				);
			});
	...
	useEffect(() => {
		if (selectedAnswerId) {
			const selected = variablesHistory.find(
				(e) => e.id === selectedAnswerId,
			);
			if (selected) {
	...
	const getCaseDescription = (stepTitle: string, parsedContent: any) => {
		switch (stepTitle) {
			case "PlanControllerAgent":
				if (
					parsedContent.subtasks_progress &&
					parsedContent.next_subtask
				) {
			...
			case "CodeAgent":
				if (parsedContent.code) {
					...
					const outputPreview = parsedContent.execution_output
						? parsedContent.execution_output.substring(0, 50) +
							(parsedContent.execution_output.length > 50
								? "..."
								: "")
						: "";
			...
			case "ShortlisterAgent":
				if (parsedContent.result) {
					...
					const apiName =
						topResult?.name || topResult?.title || "Unknown API";
					const truncatedName =
						apiName.length > 30
							? apiName.substring(0, 30) + "..."
							: apiName;
	...
	const renderStepContent = useCallback(
		(step: Step) => {
			try {
				...
				if (typeof step.content === "string") {
					try {
						...
						if (
							...
							console.log(
								`[${step.title}] Processing with variables...`,
							);
				...
				switch (step.title) {
					case "PlanControllerAgent":
						if (
							...
							mainElement = (
								<TaskStatusDashboard taskData={parsedContent} />
							);
						}
						break;
					case "TaskDecompositionAgent":
					...
					case "CodeAgent":
						if (parsedContent.code) {
							mainElement = (
								<CoderAgentOutput coderData={parsedContent} />
							);
						}
						break;
					case "ShortlisterAgent":
					...
					case "TaskAnalyzerAgent":
						if (parsedContent && Array.isArray(parsedContent)) {
							mainElement = (
								<AppAnalyzerComponent appData={parsedContent} />
							);
						}
						break;
					case "PlannerAgent":
						if (parsedContent) {
							mainElement = (
								<ActionAgent agentData={parsedContent} />
							);
						}
						break;
					...
					case "QaAgent":
						if (parsedContent) {
							mainElement = (
								<QaAgentComponent qaData={parsedContent} />
							);
						}
						break;
					case "Answer":
					case "FinalAnswerAgent":
						if (parsedContent) {
							...
							console.log(
								"Answer/FinalAnswerAgent - answerText:",
								answerText,
							);
				...
				console.log(
					`Failed to parse JSON for step ${step.title}:`,
					error,
				);
				return null;
			}
	...
	const finalAnswerSteps = currentSteps.filter(
		(step) =>
			step.title === "FinalAnswerAgent" || step.title === "FinalAnswer",
	);

	// Show SuggestHumanActions as active if it's not marked as completed
	const userActionSteps = currentSteps.filter(
		(step) => step.title === "SuggestHumanActions" && !step.completed,
	);

	// Include completed SuggestHumanActions in reasoning steps
	const reasoningSteps = currentSteps.filter(
		(step) =>
			step.title !== "FinalAnswerAgent" &&
					...
					<div>
						...
						{reasoningSteps.length > 0 && (
							...
							>
								...
								<div
									style={{
										maxHeight: isReasoningCollapsed
											? "0"
											: "10000px",
										overflow: "hidden",
										transition:
