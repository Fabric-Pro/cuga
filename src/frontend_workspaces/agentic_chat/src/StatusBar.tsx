...
export function StatusBar({ threadId }: StatusBarProps) {
	...
	const [internalToolsCount, setInternalToolsCount] = useState<
		Record<string, number>
	>({});
	const [mode, setMode] = useState<"fast" | "balanced">("fast");
	const [agentMode, setAgentMode] = useState<"supervisor" | "single">(
		"supervisor",
	);
	...
	const [visibleItems, setVisibleItems] = useState<Set<string>>(
		new Set(["tools", "mode", "agents", "connection"]),
	);
	const statusBarRef = useRef<HTMLDivElement>(null);
	const agentsPopupTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	const examplesPopupTimeoutRef = useRef<ReturnType<
		typeof setTimeout
	> | null>(null);
	const modePopupTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	...
	return (
		<>
				...
				>
					...
					>
						...
						<div className="config-modal-content">
							...
							>
								Choose which agent you want to communicate with
								directly:
							</p>
							<div
								style={{
							...
							>
								{subAgents.filter((a) => a.enabled).length ===
								0 ? (
									<div
										style={{
											padding: "24px",
									...
									>
										No active agents available. Enable
										agents in Sub Agents configuration.
									</div>
								) : (
									subAgents
										.filter((a) => a.enabled)
			...
			<div className="status-bar" ref={statusBarRef}>
				<div className="status-bar-left">{/* Left spacer */}</div>
							...
							>
								<div className="examples-popup-header">
									...
									<span className="examples-count">
										{exampleUtterances.length} examples
									</span>
								</div>
														...
														<div className="tool-group-header">
															...
															<div className="tool-group-stats">
																...
																{internalToolsCount[
																	...
																	undefined && (
																	<span className="tool-group-internal">
																		Internal:{" "}
																		{
																			internalToolsCount[
																				type.toLowerCase()
							...
							{showModePopup && (
								...
								>
									...
									>
										...
										>
											Clone the repo to experience full
											features of CUGA:
										</div>
										<a
										...
										>
											<span>
												github.com/cuga-project/cuga-agent
											</span>
											<svg
												width="12"
												height="12"
									...
									<div className="agents-list">
										{agentMode === "supervisor" ? (
											subAgents.length === 0 ? (
												...
												<>
													<div className="agents-info-box">
														<span className="agents-info-label">
											...
											<div className="agents-info-box single-mode">
												...
												{selectedAgent ? (
													<>
														<span className="single-agent-label">
															Talking with:{" "}
															{
