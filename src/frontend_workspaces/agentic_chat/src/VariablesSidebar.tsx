					...
					<div className="variables-sidebar-content">
						{history.length > 0 && (
							<div className="variables-history-info">
								Viewing:{" "}
								{history.find((h) => h.id === selectedAnswerId)
									?.title || "Latest turn"}
								<span className="history-count">
									{history.length} turns total
								</span>
							</div>
						)}
						<div className="variables-list">
							{variableKeys.length === 0 && history.length > 0 ? (
								<div className="no-variables-message">
									...
									<p>
										Select a previous turn from the dropdown
										above to view its variables.
									</p>
								</div>
							) : (
								variableKeys.map((varName) => {
									...
									return (
										...
										>
											...
											{variable.count_items !==
												undefined &&
												variable.count_items > 1 && (
											...
											<div className="variable-preview">
												{variable.value_preview
													? variable.value_preview.substring(
															0,
															80,
														) +
														(variable.value_preview
