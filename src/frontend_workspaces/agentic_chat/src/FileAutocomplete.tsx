...
export function FileAutocomplete({
	...
	const getInputPosition = () => {
		...
		const textarea = document.querySelector(
			'.WAC__TextArea-textarea, textarea, input[type="text"]',
	...
	const handleFileSelect = useCallback(
		(file: { name: string; path: string }) => {
			...
			const textAfterSearchTerm = currentValue.substring(
				lastAtIndex + 1 + searchTerm.length,
			);
	...
	useEffect(() => {
		...
		const setupCarbonListeners = () => {
			...
			const carbonChat = document.querySelector(
				"cds-aichat-react",
			) as any;

			// Also check for custom chat textarea
			const customChatTextarea =
				document.getElementById("main-input_field");
			...
			if (carbonChat || customChatTextarea) {
				...
				const handleCarbonInput = (event: any) => {
					...
					if (
						...
						if (
							tryHandleValue(active.value || active.textContent)
						) {
							return;
						}
					}
				...
				setTimeout(() => {
					...
					textareas.forEach((textarea) => {
						if (
							!textarea.hasAttribute("data-autocomplete-listener")
						) {
							textarea.setAttribute(
								"data-autocomplete-listener",
								"true",
							);
				...
				const handleDocumentInput = (e: Event) => {
					...
					if (
						target &&
						target.hasAttribute("data-autocomplete-listener")
					) {
						return;
					}
					if (
				...
				return () => {
					...
					if (carbonChat) {
						carbonChat.removeEventListener(
							"input",
							handleCarbonInput,
						);
						carbonChat.removeEventListener(
