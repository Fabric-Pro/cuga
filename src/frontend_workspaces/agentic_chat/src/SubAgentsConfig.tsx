...
export default function SubAgentsConfig({ onClose }: SubAgentsConfigProps) {
	...
	const [appToolsCache, setAppToolsCache] = useState<
		Record<string, AppTool[]>
	>({});
	const [loadingApps, setLoadingApps] = useState(false);
	const [showAddAgentModal, setShowAddAgentModal] = useState(false);
	const [newAgentSource, setNewAgentSource] =
		useState<AgentSourceType>("direct");
	const [newAgentUrl, setNewAgentUrl] = useState("");
	const [newAgentName, setNewAgentName] = useState("");
	const [newAgentEnvVars, setNewAgentEnvVars] = useState<
		Array<{ key: string; value: string }>
	>([]);
	const [newAgentStreamType, setNewAgentStreamType] = useState<
		"http" | "sse"
	>("http");
	...
	const loadAppTools = async (appName: string) => {
		...
		try {
			const response = await fetch(
				`/api/apps/${encodeURIComponent(appName)}/tools`,
			);
			if (response.ok) {
				const data = await response.json();
	...
	const createAgent = () => {
		...
		const newAgent: SubAgent = {
			...
			name:
				newAgentSource === "a2a" && newAgentName
					? newAgentName
					: "New Agent",
			role: "Assistant",
			description: "",
	...
	const unassignApp = (agentId: string, appName: string) => {
		...
		if (agent) {
			updateAgent(agentId, {
				assignedApps: agent.assignedApps.filter(
					(a) => a.appName !== appName,
				),
			});
		}
					...
					<div className="config-card">
						...
						<div className="config-form">
							<div className="form-group">
								...
								<small>
									Supervisor mode delegates tasks to
							...
							{config.mode === "supervisor" && (
								<div className="form-group">
									...
									<small>
										How the supervisor coordinates
										sub-agents
									</small>
								</div>
							)}
						...
						<div className="config-card">
							...
							<div className="sources-list">
								{config.subAgents.map((agent) => {
									...
									const enabledTools = agent.tools.filter(
										(t) => t.enabled,
									).length;
									const totalAppTools =
										agent.assignedApps.reduce(
											(sum, app) =>
												sum +
												app.tools.filter(
													(t) => t.enabled,
												...
												<div className="agent-config-details">
															...
															<div className="source-info-card">
																...
																{agent.source
																	...
																	<div className="source-info-row">
																		...
																		<span>
																			{agent.source.streamType.toUpperCase()}
																		</span>
																	</div>
																)}
																{agent.source
													...
													<div className="form-group">
														<div className="form-group-header">
															...
															<span className="tools-count-small">
																{enabledTools}/
																{
																	agent.tools
																		.length
							...
							{config.subAgents.length === 0 && (
								<div className="empty-state">
									<p>
										No sub-agents configured. Click "Add
										Agent" to create one.
									</p>
								</div>
							)}
