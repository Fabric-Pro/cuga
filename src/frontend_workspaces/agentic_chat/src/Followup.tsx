...
export const FollowupAction = ({
	...
	const theme =
		colorThemes[color as keyof typeof colorThemes] || colorThemes.primary;
	...
	const handleTextSubmit = () => {
		...
		if (
			validation_pattern &&
			!new RegExp(validation_pattern).test(response)
		) {
		...
		if (max_length && response.length > max_length) {
			console.error(
				`Response must be no more than ${max_length} characters`,
			);
			return;
		}
	...
	const handleSelectChange = (value: string) => {
		...
		if (type === "select") {
			const selectedOptions = (options || []).filter((opt) =>
				newSelection.includes(opt.value),
			);
			handleSubmit({
					...
					<div className="space-y-3">
						...
						{max_length && (
							<div className="text-xs text-gray-500 text-right">
								<span
									className={
										response.length > max_length * 0.8
											? "text-orange-500"
											: ""
									}
								>
