...
export default function MemoryConfig({ onClose }: MemoryConfigProps) {
	...
	const [expandedTrajectory, setExpandedTrajectory] = useState<string | null>(
		null,
	);
	...
	const addBlockedItem = () => {
		if (
			newBlockedItem.trim() &&
			!config.blockedMemoryItems.includes(newBlockedItem.trim())
		) {
			setConfig({
				...config,
				blockedMemoryItems: [
					...config.blockedMemoryItems,
					newBlockedItem.trim(),
				],
			});
			setNewBlockedItem("");
	...
	const removeBlockedItem = (item: string) => {
		setConfig({
			...config,
			blockedMemoryItems: config.blockedMemoryItems.filter(
				(i) => i !== item,
			),
		});
	};
	...
	const toggleTrajectory = (id: string) => {
		setConfig({
			...
			saveAndReuse: {
				...config.saveAndReuse,
				savedTrajectories: config.saveAndReuse.savedTrajectories.map(
	...
	const deleteTrajectory = (id: string) => {
		setConfig({
			...
			saveAndReuse: {
				...config.saveAndReuse,
				savedTrajectories: config.saveAndReuse.savedTrajectories.filter(
					(t) => t.id !== id,
				),
			},
		});
					...
					<div className="config-card">
						...
						<div className="config-form">
							...
							<div className="form-group">
								...
								<small>
									Allow the agent to remember context across
									conversations
								</small>
							</div>
							...
							<div className="form-group">
								<label className="checkbox-label">
									...
									/>
									<span>
										Enable Fact Storage and Retrieval
									</span>
								</label>
								<small>
									Store and retrieve important facts like IDs,
									key values, and persistent information
								</small>
							</div>
							...
							<div className="form-group">
								<label className="checkbox-label">
									...
									/>
									<span>
										Enable Learning from Failures and Tip
										Injection
									</span>
								</label>
								<small>
									Learn from past failures and analyze
									trajectories to improve future task
									performance
								</small>
							</div>
							...
							<div className="form-group">
								...
								{config.blockedMemoryItems.length === 0 ? (
									<div className="policies-empty">
										No blocked items. Add items that the
										agent should never remember.
									</div>
								) : (
									<div className="policies-list">
										{config.blockedMemoryItems.map(
											(item, index) => (
												<div
													key={index}
								...
								<small>
									Items the agent is not allowed to remember
									(e.g., sensitive information, passwords)
								</small>
							</div>
						</div>
					</div>
					...
					<div className="config-card">
						...
						<div className="config-form">
							<div className="form-group">
								...
								<small>
									Allow agent to save successful task
									trajectories as reusable tools
								</small>
							</div>
							...
							<div className="form-group">
								<label className="checkbox-label">
									<input
										type="checkbox"
										checked={
											config.saveAndReuse.autoGeneralize
										}
										onChange={(e) =>
											setConfig({
												...config,
								...
								<small>
									Automatically identify patterns and create
									reusable tools from successful tasks
								</small>
							</div>

							<div className="form-group">
								<label>
									Min. Successful Runs Before Saving
								</label>
								<input
									type="number"
									value={
										config.saveAndReuse.minSuccessfulRuns
									}
									onChange={(e) =>
										setConfig({
											...config,
								...
								/>
								<small>
									Number of successful executions before
									suggesting trajectory as reusable tool
								</small>
							</div>
							...
							<div className="form-group">
								<label className="checkbox-label">
									<input
										type="checkbox"
										checked={
											config.saveAndReuse.requireApproval
										}
										onChange={(e) =>
											setConfig({
												...config,
								...
								<small>
									Ask for permission before saving new
									trajectories as tools
								</small>
							</div>
							...
							>
								...
								>
									Saved Trajectories (
									{
										config.saveAndReuse.savedTrajectories
											.length
									}
									)
								</label>
								{config.saveAndReuse.savedTrajectories
									.length === 0 ? (
									<div className="policies-empty">
										No saved trajectories yet. When enabled,
										successful task patterns will appear
										here.
									</div>
								) : (
									<div
									...
									>
										{config.saveAndReuse.savedTrajectories.map(
											(trajectory) => {
												const isExpanded =
													expandedTrajectory ===
													trajectory.id;
								...
								>
									Trajectories are automatically learned from
