...
export default function ToolsConfig({ onClose }: ToolsConfigProps) {
			...
			if (appsResponse.ok) {
				...
				for (const app of apps) {
					try {
						...
						console.warn(
							`Failed to load tools for app ${app.name}:`,
							error,
						);
						appTools[app.name] = [];
					}
	...
	const removeArg = (serverName: string, index: number) => {
		const currentServer = configData.mcpServers?.[serverName];
		const newArgs = (currentServer?.args || []).filter(
			(_, i) => i !== index,
		);
		updateMcpServer(serverName, "args", newArgs);
	};
				...
				<div className="config-modal-content">
					{activeTab === "apps" && (
						<div className="apps-section">
							...
							{(configData.apps || []).length === 0 &&
							!loading ? (
								<div className="empty-state">
									<p>
										No apps available. Make sure the
					...
					{activeTab === "mcpServers" && (
						<div className="mcp-servers-section">
							...
							{Object.entries(configData.mcpServers || {}).map(
								([serverName, server]) => (
									<div
										key={serverName}
							...
							{Object.keys(configData.mcpServers || {}).length ===
								...
								<div className="empty-state">
									<p>
										No MCP servers configured. Click "Add
					...
					{activeTab === "services" && (
						<div className="services-section">
							...
							!loading ? (
								<div className="empty-state">
									<p>
										No services configured. Services are
										defined in the YAML configuration file.
									</p>
								</div>
							) : (
								<div className="services-list">
									{(configData.services || []).map(
										(serviceObj, index) => {
