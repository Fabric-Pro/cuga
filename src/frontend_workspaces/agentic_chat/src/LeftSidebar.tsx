...
export function LeftSidebar({
	...
	useEffect(() => {
		console.log(
			"[LeftSidebar] globalVariables updated:",
			Object.keys(globalVariables).length,
			"keys",
		);
		console.log(
			"[LeftSidebar] variablesHistory updated:",
			variablesHistory.length,
			"items",
		);
	...
	const createNewConversation = async (customTitle?: string) => {
		console.log(
			"[LeftSidebar] createNewConversation called with title:",
			customTitle,
		);
		const newConv = {
			id: `conv-${Date.now()}`,
			...
			if (response.ok) {
				...
				setConversations((prevConversations) =>
					prevConversations.map((conv) =>
						conv.id === newConv.id
							? { ...apiConv, id: newConv.id }
							: conv,
					),
				);
			} else {
				console.warn(
					"[LeftSidebar] API call failed but local conversation added",
				);
			}
		} catch (error) {
			console.warn(
				"[LeftSidebar] API error but local conversation added:",
				error,
			);
		}
	};
	...
	if (!isExpanded) {
		return (
			...
			>
				...
				<div className="sidebar-floating-count">
	...
	return (
		...
		>
			<div className="left-sidebar-header">
				<div className="left-sidebar-tabs">
					...
					<button
						...
						onClick={() =>
							onTabChange ? onTabChange("variables") : null
						}
					>
						<Database size={16} />
						<span>Variables</span>
						...
						>
							...
							<div className="sidebar-tab-info-tooltip">
								Variables are the results from task execution.
								Ask any question about the variables and CUGA
								will respond.
							</div>
						</div>
					</button>
					{ENABLE_SAVED_FLOWS && (
						<button
							...
							onClick={() =>
								onTabChange ? onTabChange("savedflows") : null
							}
						>
							<Workflow size={16} />
			...
			{hoveredFlowId && tooltipPosition && (
				...
				>
					Saved from a previous conversation where you completed a
					similar task using CRM, filesystem, and email tools. Reuse
					this flow to repeat the same pattern with different
					parameters.
				</div>
			)}
		</div>
