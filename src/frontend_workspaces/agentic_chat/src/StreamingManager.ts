...
export class StreamingManager {
	...
	private async simulateFakeStream(query: string): Promise<void> {
		...
		try {
			...
			// Add initial step
			if (!this.shouldStop) {
				this.options.onAddStep(
					"simple_text",
					"Simulating stream from JSON file...",
				);
			}
			...
			for (
				let i = 0;
				i < fakeStreamData.steps.length && !this.shouldStop;
				i++
			) {
	...
	static setFakeStreamConfig(
		enabled: boolean,
		file: string = FAKE_STREAM_FILE,
		delay: number = FAKE_STREAM_DELAY,
	) {
		...
		console.log(
			`Fake stream config: enabled=${enabled}, file=${file}, delay=${delay}`,
		);
	}
}
