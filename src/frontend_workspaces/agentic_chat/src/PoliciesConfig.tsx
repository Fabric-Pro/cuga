...
export default function PoliciesConfig({ onClose }: PoliciesConfigProps) {
	...
	const [activeTab, setActiveTab] = useState<
		...
		| "answer"
	>("intent");
	const [toolsSubTab, setToolsSubTab] = useState<"guards" | "enrichments">(
		"guards",
	);
	...
	const updateSubAgentPolicy = (
		id: string,
		updates: Partial<SubAgentPolicy>,
	) => {
		setConfig({
			...config,
	...
	const updateToolEnrichment = (
		id: string,
		updates: Partial<ToolEnrichment>,
	) => {
		setConfig({
			...config,
	...
	const removePolicy = (
		...
		switch (type) {
			case "intent":
				setConfig({
					...config,
					intentPolicies: config.intentPolicies.filter(
						(p) => p.id !== id,
					),
				});
				break;
			...
			case "subagent":
				setConfig({
					...config,
					subAgentPolicies: config.subAgentPolicies.filter(
						(p) => p.id !== id,
					),
				});
				break;
			...
			case "toolenrichments":
				setConfig({
					...config,
					toolEnrichments: config.toolEnrichments.filter(
						(p) => p.id !== id,
					),
				});
				break;
			case "answer":
				setConfig({
					...config,
					answerPolicies: config.answerPolicies.filter(
						(p) => p.id !== id,
					),
				});
				break;
		...
		<div className="config-modal-overlay" onClick={onClose}>
			<div className="config-modal" onClick={(e) => e.stopPropagation()}>
				...
				<div className="config-modal-content">
					<div className="config-card">
						...
						<div className="config-form">
							<div className="form-group">
								...
								<small>
									Master switch for all policy enforcement
								</small>
							</div>
					...
					{(activeTab === "toolguards" ||
						activeTab === "toolenrichments") &&
						renderToolsSection()}
			...
			<div className="config-card">
								...
								{isExpanded && (
									<div className="agent-config-details">
										<div className="form-group">
											...
											/>
											<small>
												Keywords or phrases that trigger
												this policy
											</small>
										</div>
				...
				{config.intentPolicies.length === 0 && (
					<div className="empty-state">
						<p>
							No intent policies configured. Click "Add Intent
		...
		return (
			<div className="config-card">
				...
				{config.sopPolicies.length === 0 && (
					<div className="empty-state">
						<p>
							No SOPs configured. Click "Add SOP" to create one.
						</p>
					</div>
				)}
			</div>
		...
		return (
			<div className="config-card">
								...
								{isExpanded && (
									<div className="agent-config-details">
										...
										<div className="form-group">
											...
											/>
											<small>
												Tools this sub-agent is allowed
												to use
											</small>
										</div>
									</div>
				...
				{config.subAgentPolicies.length === 0 && (
					<div className="empty-state">
						<p>
							No sub-agent policies configured. Click "Add
							Sub-Agent Policy" to create one.
						</p>
					</div>
				)}
			</div>
		...
		return (
			<div className="config-card">
								...
								{isExpanded && (
									<div className="agent-config-details">
										...
										<div className="form-group">
											...
											/>
											<small>
												General guidance for using this
												app (separate from rules and
												permissions)
											</small>
										</div>
				...
				{config.appPolicies.length === 0 && (
					<div className="empty-state">
						<p>
							No app policies configured. Click "Add App Policy"
							to create one.
						</p>
					</div>
				)}
		...
		return (
			<>
									...
									<div className="agent-config-details">
										...
										<div className="form-group">
											...
											>
												...
												<option value="input_validation">
													Input Validation
												</option>
												<option value="output_filter">
													Output Filter
												</option>
												<option value="approval_required">
													Approval Required
												</option>
												<option value="time_restriction">
													Time Restriction
												</option>
											</select>
											<small>
										...
										{guard.guardType ===
											...
											<div className="form-group">
												...
												<input
													type="text"
													value={
														guard.config.inputValidationRules?.join(
															", ",
														) || ""
													}
													onChange={(e) =>
										...
										{guard.guardType ===
											...
											<div className="form-group">
												...
												<input
													type="text"
													value={
														guard.config.outputFilterPatterns?.join(
															", ",
														) || ""
													}
													onChange={(e) =>
										...
										{guard.guardType ===
											...
											<div className="form-group">
												...
												<input
													type="text"
													value={
														guard.config.allowedTimeRanges
															?.map(
																(r) =>
																	`${r.start}-${r.end}`,
															)
				...
				{config.toolGuards.length === 0 && (
					<div className="empty-state">
						<p>
							No tool guards configured. Click "Add Tool Guard" to
							create one.
						</p>
					</div>
				)}
		...
		return (
			<>
								...
								{isExpanded && (
									<div className="agent-config-details">
										...
										<div className="form-group">
											...
											<textarea
												value={
													enrichment.postProcessingRules ||
													""
												}
												onChange={(e) =>
													updateToolEnrichment(
										...
										<div className="form-group">
											...
											<div className="policies-list">
												{enrichment.customInstructions.map(
													(instruction, index) => (
														<div
															key={index}
															className="policy-item"
														>
															...
															<button
																...
																onClick={() => {
																	const newInstructions =
																		enrichment.customInstructions.filter(
																			(
																				_,
																				i,
																			) =>
													...
													(example, index) => (
														...
														>
															...
															<button
																...
																onClick={() => {
																	const newExamples =
																		enrichment.exampleUsages.filter(
																			(
																				_,
																				i,
																			) =>
													...
													(practice, index) => (
														...
														>
															...
															<button
																...
																onClick={() => {
																	const newPractices =
																		enrichment.bestPractices.filter(
																			(
																				_,
																				i,
																			) =>
				...
				{config.toolEnrichments.length === 0 && (
					<div className="empty-state">
						<p>
							No tool enrichments configured. Click "Add Tool
							Enrichment" to create one.
						</p>
					</div>
				)}
			</>
		...
		return (
			<div className="config-card">
								...
								{isExpanded && (
									<div className="agent-config-details">
										...
										<div className="form-group">
											...
											<small>
												Additional rules the agent must
												follow when responding
											</small>
											<div className="policies-list">
												{policy.customInstructions.map(
													(instruction, index) => (
														<div
															key={index}
															className="policy-item"
														>
															...
															<button
																...
																onClick={() => {
																	const newInstructions =
																		policy.customInstructions.filter(
																			(
																				_,
																				i,
																			) =>
										...
										<div className="form-group">
											...
											<small>
												Phrases that must not appear in
												responses
											</small>
											<div className="policies-list">
												{policy.forbiddenPhrases.map(
													(phrase, index) => (
														<div
															key={index}
															className="policy-item"
														>
															...
															<button
																...
																onClick={() => {
																	const newPhrases =
																		policy.forbiddenPhrases.filter(
																			(
																				_,
																				i,
																			) =>
				...
				{config.answerPolicies.length === 0 && (
					<div className="empty-state">
						<p>
							No answer policies configured. Click "Add Answer
							Policy" to create one.
						</p>
					</div>
				)}
