...
async ([parent_bid, bid_attr_name, tags_to_mark]) => {
	...
	while (i < elements.length) {
		...
		switch (tags_to_mark) {
			...
			case "standard_html":
				if (
					!elem.tagName ||
					!html_tags.has(elem.tagName.toLowerCase())
				) {
		...
		if (elem.hasAttribute(bid_attr_name)) {
			...
			if (browsergym_first_visit) {
				throw new Error(
					`Attribute ${bid_attr_name} already used in element ${elem.outerHTML}`,
				);
			}
			...
			if (all_bids.has(elem_global_bid)) {
				console.log(
					`BrowserGym: duplicate bid ${elem_global_bid} detected, generating a new one`,
				);
		...
		if (["self", "child"].includes(whoCapturesCenterClick(elem))) {
			...
			if (
				set_of_marks_tags.has(elem.tagName.toLowerCase()) ||
				elem.onclick != null ||
	...
	try {
		await until(
			...
			visibility_marking_timeout,
		);
	} catch {
		warning_msgs.push(
			`Frame marking: not all elements have been visited by the intersection_observer after ${visibility_marking_timeout} ms`,
		);
	}
