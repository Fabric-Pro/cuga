...
export async function* streamQuery(
	...
	console.log(
		`[CUGA Client] Sending stream request to ${url.toString()} with thread_id: ${request.thread_id}, history: ${request.history?.length || 0} messages, hasCredentials: ${!!credentials}`,
	);
	...
	if (!response.ok) {
		...
		console.error(
			`[CUGA Client] Request failed: ${response.status} ${response.statusText}`,
			errorText,
		);
		throw new Error(
			`CUGA query failed: ${response.status} ${response.statusText} - ${errorText}`,
		);
	}
...
export async function* resumeExecution(
	...
	if (!response.ok) {
		throw new Error(
			`CUGA resume failed: ${response.status} ${response.statusText}`,
		);
	}
